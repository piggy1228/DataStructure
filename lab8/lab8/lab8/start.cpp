...
int back_to_original(vector<vector<bool> > const& blocked_grid,int x,int y){
    if (x==0 and y==0) {
        return 1;
    }
    else if (x==0){
        if (blocked_grid[0][y-1]) {
            return 0;
        }
        
        return  back_to_original(blocked_grid,0, y-1);
    }
    else if (y==0){
        if (blocked_grid[x-1][0]) {
            return 0;
        }

        return back_to_original(blocked_grid,x-1, 0);
    }

    else{
        if (! blocked_grid[x-1][y] and !blocked_grid[x][y-1]) {
            
            return back_to_original(blocked_grid, x-1, y) + back_to_original(blocked_grid, x, y-1);
        }
        else if(blocked_grid[x-1][y] && !blocked_grid[x][y-1]){
            return back_to_original(blocked_grid, x, y-1);
        }
        else if(blocked_grid[x-1][y] && blocked_grid[x][y-1]){
            return 0;
        }
        else{
            return back_to_original(blocked_grid, x-1, y);
        }
    }
}
...
int back_to_original(vector<vector<Point>> &ways, vector<Point> one_way,vector<vector<bool> > const& blocked_grid,int x,int y){
    one_way.push_back(Point(x, y));
    if (x==0 and y==0) {
        ways.push_back(one_way);
        return 1;
    }
    else if (x==0){
        if (blocked_grid[0][y-1]) {
        ...
        return  back_to_original(ways,one_way,blocked_grid,0, y-1);
    }
    else if (y==0){
        if (blocked_grid[x-1][0]) {
            return 0;
        }
        return back_to_original(ways,one_way,blocked_grid,x-1, 0);
    }
    
    else{
        if (! blocked_grid[x-1][y] and !blocked_grid[x][y-1]) {

            return back_to_original(ways,one_way,blocked_grid, x-1, y) + back_to_original(ways,one_way,blocked_grid, x, y-1);
        }
        else if(blocked_grid[x-1][y] && !blocked_grid[x][y-1]){
            return back_to_original(ways,one_way,blocked_grid, x, y-1);
        }
        else if(blocked_grid[x-1][y] && blocked_grid[x][y-1]){
            return 0;
        }
        else{
            return back_to_original(ways,one_way,blocked_grid, x-1, y);
        }
    }
}
...
int main(int argc, char* argv[]) {
  ...
  // Start here with your code...
    ...
    int a=back_to_original(blocked_grid, start_x, start_y);
    std::cout<<"different paths number is: "<<a<<std::endl;
 
    vector<vector<Point>> ways;
    vector<Point> one_way;
    int b = back_to_original(ways, one_way, blocked_grid, start_x, start_y);
    std::cout<<"different paths number is: "<<b<<std::endl;
    ...
    for (int i=0; i<ways.size(); i++) {
        for (int j=0; j<ways[i].size()-1; j++) {
            ...
            std::cout<<"("<<ways[i][j].x<<" , "<<ways[i][j].y<<") ->";
        }
        std::cout<<"("<<ways[i][ways[i].size()-1].x<<" , "<<ways[i][ways[i].size()-1].y<<")\n";
    }

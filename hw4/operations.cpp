...
int arithmetic_operations() {
  ...
  float zeropointone = multidivide(f*10, a, a, a, 1)/float(a);
...
int list_operations() {
  ...
  for(std::list<int>::iterator itr = l500.begin(); itr != l500.end(); ++itr) {
    if(*itr % factor == 0 || *itr % factor2 == 0) {
        if(std::find(l500.begin(), l500.end(), *itr) != l500.end()){
            itr = l500.erase(itr);
            itr -- ;
        }
    }

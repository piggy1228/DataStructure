...
int main() {
  ...
  assert(set1.size() == set2.size() + 2);
    ...
    set3.insert(13);
    set3.insert(15);
    cout<<"exactly balance tree:\n";
    set3.print_as_sideways_tree(cout);
  // Test the iterators!
    ds_set<int>::iterator itr = set3.end();
    while(itr !=set3.begin()){
        itr--;
        cout<<(*itr)<<"\n";
    }
    
    cout<<"accumulated:\n";
    cout<<set3.accumulate(0)<<endl;
    cout<<set1.accumulate("hi")<<endl;

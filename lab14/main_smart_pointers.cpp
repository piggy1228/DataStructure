...
void deleteAll(Balloon* person,std::set<Balloon*> &accesible){
    std::set<Balloon*>::iterator itr = accesible.find(person);
    if (itr!= accesible.end()) {
        return;
    }
    accesible.insert(person);
    int num = person->numRopes();
    //std::cout<<"1\n";
    //std::cout<<num<<std::endl;
    for (int i = 0; i<num; i++) {
        //std::cout<<"2\n";
        Balloon* p = person->getRope(i);
        deleteAll(p, accesible);
    }
}

void deleteAll(Balloon* &person){
    std::set<Balloon*> accesible;
    
    deleteAll( person,accesible);
    
    for (std::set<Balloon*>::iterator itr = accesible.begin(); itr != accesible.end(); itr++) {
        delete (*itr);
    }
}
